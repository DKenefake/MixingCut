...
pub fn get_improving_children(x_0: &Array1<f64>, Q: &CsMat<f64>) -> Vec<(f64, Array1<f64>)> {
    ...
    for i in 0..Q.rows() {
        obj_delta[i] =
            (2.0 * d[i]).mul_add(obj_delta[i], d[i] * d[i] * Q.get(i, i).unwrap_or(&0.0));
    }
...
pub fn beam_search(
    ...
    let mut beam_candidates: Vec<_> = candidates
        .iter()
        .flat_map(|x| get_improving_children(x, Q))
        .collect();

    while !beam_candidates.is_empty() {
        ...
        beam_candidates = beam_candidates
            .par_iter()
            .flat_map(|(_, x)| get_improving_children(x, Q))
            .collect();
        ...
        beam_candidates = beam_candidates.iter().take(beta).cloned().collect();
    }
